...
function Admin() {
    ...
    const deleteUser =(userId)=>{
        axios.post('http://localhost:5000/admin/deleteuser',{userId},{withCredentials:true}).then(res=>{
    ...
    useEffect(() => {
        const fetchData = async () => {
          try {
            const usersResponse = await axios.get('http://localhost:5000/getusers');
            const usersData = usersResponse.data;
            setUsers(usersData);
      
            const token = localStorage.getItem('admin');
            const { data } = await axios.post('http://localhost:5000/admin/auth', { token }, { withCredentials: true });
            console.log(data.token);
      
            if (data.token === 'valid') {
              // Token is valid, continue with other logic
            } else {
...
<td>
    <button className="button_one">Edit</button>
